...
extern uint8_t MFRC522_ToCard(uint8_t command, uint8_t *sendData, uint8_t sendLen,
...
extern uint8_t MFRC522_Auth(uint8_t authMode, uint8_t BlockAddr, uint8_t *Sectorkey,
                            uint8_t *serNum);
...
//  unsigned char writeCommand[1];
    unsigned char readValue[1] = {0};

    //writeCommand[0] = data;
    SPI_I2S_SendData(SPI1, data);
    while (RESET == SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_TXE));
    readValue[0] = (uint8_t)SPI_I2S_ReceiveData(SPI1);
                       ...
                       uint16_t *backLen)
{
    ...
    MFRC522_ClearBitMask(MFRC522_REG_BIT_FRAMING,
                         0x80);                                                                // StartSend=0
...
void MFRC522_CalculateCRC(uint8_t  *pIndata, uint8_t len, uint8_t *pOutData)
{
    ...
    MFRC522_ClearBitMask(MFRC522_REG_DIV_IRQ,
                         0x04);                                                    // CRCIrq = 0
    MFRC522_SetBitMask(MFRC522_REG_FIFO_LEVEL,
                       0x80);                                                   // Clear the FIFO pointer
